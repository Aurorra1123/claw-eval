...
| D | **ClawEvalEnv 不实现 AOrchestra `Environment` ABC** — 缺 `get_basic_info()` + `step(action)`。两个 runtime 都踩。React 路径每次 attempt 报 `'ClawEvalEnv' object has no attribute 'get_basic_info'`,Pi 路径不 timeout 但 sub-agent 立刻 error → 终止 | ❌ **未修,Phase 4 latent bug** | — |
| (claw-eval) | `_build_aorchestra_tool_schemas(env)` 在 Plan v2 里被标 TODO 直接返 `{}`,导致 tool_schemas plumbing 链路有但数据源为空 | ✅ 已修 | claw-eval `821f41e` |

### T077 e2e 实测数字

| Runtime | wall_time | attempts | sub_cost | delegate_count | task_score | scores |
|---|---|---|---|---|---|---|
| react(今天复跑) | 110s | 15 | 0.0 | 0 | **0.20** | completion=0.0 robustness=1.0 communication=0.0 safety=1.0 |
| pi(Bug A/B/C 修完后) | 260s | 15 | 0.0 | 0 | **0.20** | completion=0.0 robustness=1.0 communication=0.0 safety=1.0 |

**关键 finding**:Wave 4-D 自报的"react baseline 0.28"**是 Phase 4 时的旧数字,Phase 5 改造之后跑不出来了**。今天复跑 react 也是 0.20。`0.20 = robustness(1.0)×0.2 + safety(1.0)×?`(具体权重看 grader)— 实际上 **sub-agent 一次都没真的工作**,两个 runtime 都被 Bug D 卡死。Wave 4-D 那个 0.28 大概率也是 sub-agent 完全没工作,只是 robustness/safety 的 baseline 分。
